#!/usr/bin/env python3
"""
BrainDrive Ollama Plugin Lifecycle Manager (New Architecture)

This script handles install/update/delete operations for the BrainDrive Ollama plugin
...
try:
    ...
    try:
        ...
        raise ImportError("BrainDrive Ollama plugin requires the new architecture BaseLifecycleManager")


class BrainDriveOllamaLifecycleManager(BaseLifecycleManager):
    """Lifecycle manager for BrainDrive Ollama plugin using new architecture"""
    
    def __init__(self, plugins_base_dir: str = None):
        """Initialize the lifecycle manager"""
        # Plugin-specific data for the dedicated Ollama integration
        self.plugin_data = {
            ...
            "compatibility": "1.0.0",
            "scope": "BrainDriveOllama",
            "bundle_method": "webpack",
            "bundle_location": "dist/remoteEntry.js",
            "is_local": False,
            "long_description": "Manages Ollama server connections, model lifecycle, and chat experiences within BrainDrive.",
            "plugin_slug": "BrainDriveOllama",
            # Update tracking fields (matching plugin model)
            "source_type": "remote",
            "source_url": "https://github.com/BrainDriveAI/BrainDrive-Ollama-Plugin",
            "update_check_url": "https://github.com/BrainDriveAI/BrainDrive-Ollama-Plugin/releases/latest",
        ...
        # Modules exported by the Ollama plugin
        self.module_data = [
            {
                ...
                "required_services": {
                    "api": {
                        "methods": ["get", "post", "delete", "getSse"],
                        "version": "1.0.0"
                    },
                    "theme": {
        ...
        # Settings definitions that this plugin requires
        self.PLUGIN_SETTINGS_DEFINITIONS = [
            {
                "id": "ollama_servers_settings",
                "name": "Ollama Servers Settings",
                "description": "Persisted configuration for registered Ollama servers and credentials.",
                "category": "ollama",
                "type": "object",
        ...
        logger.info(f"BrainDriveOllama: plugins_base_dir - {plugins_base_dir}")
        if plugins_base_dir:
        ...
        logger.info(f"BrainDriveOllama: shared_path - {shared_path}")
        super().__init__(
    ...
    async def _perform_user_installation(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"BrainDriveOllama: User installation completed for {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"BrainDriveOllama: User installation failed for {user_id}: {e}")
    ...
    async def _perform_user_uninstallation(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"BrainDriveOllama: User uninstallation completed for {user_id}")
            return {
                'success': True,
                'plugin_id': plugin_id,
            ...
            logger.error(f"BrainDriveOllama: User uninstallation failed for {user_id}: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _copy_plugin_files_impl(self, user_id: str, target_dir: Path, update: bool = False) -> Dict[str, Any]:
        """
        BrainDriveOllama-specific implementation of file copying.
        ...
        try:
            ...
            logger.info(f"BrainDriveOllama: Copied {len(copied_files)} files/directories to {target_dir}")
            return {'success': True, 'copied_files': copied_files}
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error copying plugin files: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _validate_installation_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        BrainDriveOllama-specific validation logic.
        This method is called by the base class during installation.
        """
        try:
            # Check for BrainDriveOllama-specific required files
            ...
            if missing_files:
                return {
                    'valid': False,
                    'error': f"BrainDriveOllama: Missing required files: {', '.join(missing_files)}"
                }
            ...
            try:
                ...
                for field in required_fields:
                    if field not in package_data:
                        return {
                            'valid': False,
                            'error': f'BrainDriveOllama: package.json missing required field: {field}'
                        }
                        
            except (json.JSONDecodeError, FileNotFoundError) as e:
                return {
                    'valid': False,
                    'error': f'BrainDriveOllama: Invalid or missing package.json: {e}'
                }
            ...
            if bundle_path.stat().st_size == 0:
                return {
                    'valid': False,
                    'error': 'BrainDriveOllama: Bundle file (remoteEntry.js) is empty'
                }
            
            logger.info(f"BrainDriveOllama: Installation validation passed for user {user_id}")
            return {'valid': True}
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error validating installation: {e}")
            return {'valid': False, 'error': str(e)}
    
    async def _get_plugin_health_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        BrainDriveOllama-specific health check logic.
        ...
        try:
            ...
            logger.info(f"BrainDriveOllama: Health check completed for user {user_id}")
            return {'healthy': True, 'details': health_info}
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error checking plugin health: {e}")
        ...
        try:
            for definition_data in self.PLUGIN_SETTINGS_DEFINITIONS:
                ...
                if not existing:
                    ...
                    logger.info(f"BrainDriveOllama: Created settings definition: {definition_data['name']}")
            
            return {'success': True}
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error ensuring settings definitions: {e}")
            return {'success': False, 'error': str(e)}
        ...
        try:
            ...
            for instance_data in settings_instances:
                ...
                if not existing:
                    ...
                    logger.info(f"BrainDriveOllama: Created settings instance: {instance_data['name']} for user {user_id}")
            
            return {'success': True, 'created_instances': created_instances}
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error creating settings instances: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _check_existing_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"BrainDriveOllama: Error checking existing plugin: {e}")
    ...
    async def _create_database_records(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"BrainDriveOllama: Created plugin, {len(modules_created)} modules, and settings for user {user_id}")
            return {
                'success': True,
                'plugin_id': plugin_id,
            ...
            logger.error(f"BrainDriveOllama: Error creating database records: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _delete_database_records(self, user_id: str, plugin_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"BrainDriveOllama: Deleted plugin and {deleted_modules} modules for user {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"BrainDriveOllama: Error deleting database records: {e}")
            return {'success': False, 'error': str(e)}
        ...
        try:
            ...
            if plugin_row:
                try:
                    ...
                    logger.warning(f"BrainDriveOllama: Failed to parse plugin config for user {user_id}")
            ...
            for row in module_result.fetchall():
                try:
                    ...
                    logger.warning(f"BrainDriveOllama: Failed to parse module config for {row[0]}")
                    continue
            ...
            logger.info(f"BrainDriveOllama: Exported user data for {user_id}")
            return {'success': True, 'data': export_data}
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error exporting user data: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _import_user_data(self, user_id: str, db: AsyncSession, user_data: Dict[str, Any]):
        ...
        try:
            if not user_data or 'data' not in user_data:
                logger.info(f"BrainDriveOllama: No user data to import for {user_id}")
                return
            ...
            logger.info(f"BrainDriveOllama: Imported user data for {user_id}")
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Error importing user data: {e}")
    ...
    async def install_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Install BrainDriveOllama plugin for specific user (compatibility method)"""
        try:
            logger.info(f"BrainDriveOllama: Starting installation for user {user_id}")
            ...
            logger.info(f"BrainDriveOllama: Created shared directory: {shared_path}")

            # Copy plugin files to the shared directory first
            copy_result = await self._copy_plugin_files_impl(user_id, shared_path)
            if not copy_result['success']:
                logger.error(f"BrainDriveOllama: File copying failed: {copy_result.get('error')}")
                return copy_result

            logger.info(f"BrainDriveOllama: Files copied successfully, proceeding with database installation")
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    if not verify_check['exists']:
                        logger.error(f"BrainDriveOllama: Installation appeared successful but verification failed")
                        return {'success': False, 'error': 'Installation verification failed'}
                    
                    logger.info(f"BrainDriveOllama: Installation verified successfully for user {user_id}")
                    result.update({
                    ...
                    logger.error(f"BrainDriveOllama: User installation failed: {result.get('error')}")
                    await db.rollback()
                    return result
                    
            except Exception as db_error:
                logger.error(f"BrainDriveOllama: Database operation failed: {db_error}")
                try:
                    await db.rollback()
                except Exception:
                    pass
            ...
            logger.error(f"BrainDriveOllama: Installation failed for user {user_id}: {e}")
            try:
                await db.rollback()
            except Exception:
                pass
    ...
    async def delete_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Delete BrainDriveOllama plugin for user (compatibility method)"""
        try:
            logger.info(f"BrainDriveOllama: Starting deletion for user {user_id}")
            ...
            if delete_result['success']:
                await db.commit()
                logger.info(f"BrainDriveOllama: Deletion completed for user {user_id}")
            
            return delete_result
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Deletion failed for user {user_id}: {e}")
            await db.rollback()
            return {'success': False, 'error': str(e)}
    
    async def get_plugin_status(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Get current status of BrainDriveOllama plugin installation (compatibility method)"""
        try:
            existing_check = await self._check_existing_plugin(user_id, db)
            ...
            logger.error(f"BrainDriveOllama: Error checking plugin status: {e}")
            return {'installed': False, 'error': str(e)}
    
    async def update_plugin(self, user_id: str, db: AsyncSession, new_version_manager: 'BrainDriveOllamaLifecycleManager') -> Dict[str, Any]:
        """Update BrainDriveOllama plugin for user (compatibility method)"""
        try:
            logger.info(f"BrainDriveOllama: Starting update for user {user_id}")
            ...
            logger.info(f"BrainDriveOllama: Update completed for user {user_id}")
            return install_result
            
        except Exception as e:
            logger.error(f"BrainDriveOllama: Update failed for user {user_id}: {e}")
            await db.rollback()
            return {'success': False, 'error': str(e)}
